...
void analysis()
{
 ...
 int N_Bin = 50;
 TAxis *xaxis = m_3pi->GetXaxis(); //untuk mencari bin center
 double chisq_total = 0;
 for (int i = 1; i<=N_Bin; i++) {
    double y_data = m_3pi->GetBinContent(i);
    double y_err = m_3pi->GetBinError(i);
    
    double  binCenter = xaxis->GetBinCenter(i);
    double y_fit = fSpectrum->Eval(binCenter);

    double chisq = ( (y_data - y_fit)*(y_data - y_fit) )/ (y_err*y_err) ;
    chisq_total = chisq_total + chisq;

    cerr<<i<<" "<<y_data<<" "<<y_err<<" "<<y_fit<<" "<<chisq_total<<endl;
  }

 double dof = 43. ; //50 dikurangi 7
 cerr<<"Hasil chi_sq per dof nya adalah"<<endl; 
 cerr<<chisq_total/dof<<endl;
